...
`make run` looks for `config.yaml` in the current working directory by default.
Since `make setup` writes to `~/.clawvisor/config.yaml`, point `CONFIG_FILE`
at it:

```bash
cd "$CLAWVISOR_REPO" && CONFIG_FILE="$HOME/.clawvisor/config.yaml" make run
```

On first run, Clawvisor:

1. Loads config from `~/.clawvisor/config.yaml` (database, vault, LLM settings)
2. Creates an `admin@local` user
3. Prints a magic link URL and opens it in the user's browser
4. Writes `~/.clawvisor/.local-session` for TUI auto-login
