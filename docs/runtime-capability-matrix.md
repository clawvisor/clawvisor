...
| --- | --- | --- | --- | --- | --- | --- |
| Runtime bootstrap/session creation | `pkg/runtime/proxy/manager.go`, `cmd/clawvisor/cmd_agent_runtime.go` | `agent run`, `runtime-env`, durable agent-token proxy auth | observe, enforce | `runtime_sessions` | session lifecycle only indirectly | U: `cmd/clawvisor/cmd_agent_runtime_test.go`; I: `pkg/runtime/proxy/manager_test.go`, `pkg/runtime/proxy/auth_test.go`; M: runtime manual pack 1/5 |
...
| Held tool-use approval/release | `pkg/runtime/proxy/tooluse_runtime.go`, `internal/runtime/review/cache.go` | Anthropic, OpenAI function-style, Codex function-style | observe, enforce | `approval_records`, review cache, leases | `runtime.tool_use.held`, `released`, `denied`, `review_failed` | I: `pkg/runtime/proxy/tooluse_runtime_test.go`; M: manual pack 6 |
...
| Lease open/close/correlation | `internal/runtime/leases`, `tooluse_runtime.go` | held/released tool execution | observe, enforce | `tool_execution_leases` | `runtime.lease.opened`, `runtime.lease.closed` | I: `pkg/runtime/proxy/tooluse_runtime_test.go`; M: manual pack 12 |
