...
func writeDaemonConfig(cfg *daemonConfig, dataDir, jwtSecret, path string) error {
	...
	fmt.Fprintf(&b, "# Auto-generated by clawvisor-server setup\n")
	// Advisory schema marker (spec 08 / PRD §11). Schema 2 = "this writer
	// emits proxy_lite.enabled explicitly", so the daemon path (which already
	// writes enabled: true below) is stamped as a schema-2 writer. Load() logs
	// a one-line notice only for configs older than this — never for ours.
	fmt.Fprintf(&b, "config_schema: %d\n", cfgpkg.CurrentConfigSchema)
