...
func sectionVaultUpstreamKeyWithPreamble(heading, preamble, provider, providerLabel, envVar, keyPrefix, dashboardPath string) string {
	...
	fmt.Fprintf(&b, "If they say yes, vault via stdin pipe (value never enters argv). The\n")
	fmt.Fprintf(&b, "`?agent_id=` parameter is required for agent-token writes — the\n")
	fmt.Fprintf(&b, "forwarder's agent-scoped-first fallback uses it transparently:\n\n")
	fmt.Fprintf(&b, "```bash\n")
	fmt.Fprintf(&b, "AGENT_ID=$(jq -r .agent_id \"$TOKEN_FILE\")\n")
	fmt.Fprintf(&b, "printf '%%s' \"$%s\" | jq -Rs '{api_key:.}' | \\\n", envVar)
	fmt.Fprintf(&b, "  curl -sS -X PUT \"$CLAWVISOR_APP_URL/api/runtime/llm-credentials/%s?agent_id=$AGENT_ID\" \\\n", provider)
	fmt.Fprintf(&b, "    -H \"Authorization: Bearer $TOKEN\" \\\n")
	fmt.Fprintf(&b, "    -H \"Content-Type: application/json\" \\\n")
	fmt.Fprintf(&b, "    --data-binary @-\n")
	fmt.Fprintf(&b, "```\n\n")
