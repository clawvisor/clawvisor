	...
	writeJSON(w, http.StatusOK, map[string]any{
		...
		"scope_drift_menu": map[string]any{
			...
			"options": map[string]any{
				...
				"d_justify":  "Emit a `<clawvisor:decision drift=\"<id>\" option=\"justify\">…</clawvisor:decision>` block in your assistant text. The block body is the justification; the same verifier re-evaluates with it threaded in. If the verifier accepts, the call is pre-cleared. If it rejects, the drift is closed — re-emit the original tool call to mint a fresh drift_id and pick (a) expand or (b) new task. (Automatic fallback to a user one-off prompt on rejection is a future enhancement; today the path is agent-driven recovery.)",
			},
