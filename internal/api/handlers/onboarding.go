...
func (h *OnboardingHandler) Setup(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Fprintf(&b, "All gateway requests through the relay **require** end-to-end encryption.\n\n")
	...
	fmt.Fprintf(&b, "}\n")
	fmt.Fprintf(&b, "```\n\n")
	fmt.Fprintf(&b, "For each gateway request:\n")
	...
	fmt.Fprintf(&b, "3. AES-256-GCM encrypt the JSON request body with a random 12-byte nonce\n")
	fmt.Fprintf(&b, "4. Send as `Content-Type: application/octet-stream` with headers:\n")
	fmt.Fprintf(&b, "   - `X-Clawvisor-E2E: aes-256-gcm`\n")
	fmt.Fprintf(&b, "   - `X-Clawvisor-Ephemeral-Key: <base64 ephemeral public key>`\n")
	fmt.Fprintf(&b, "5. Body: `base64(nonce ‖ ciphertext ‖ GCM auth tag)`\n")
	fmt.Fprintf(&b, "6. Decrypt the response the same way (response also has `X-Clawvisor-E2E` header)\n\n")
	fmt.Fprintf(&b, "If a helper is available, use it — the bundled `e2e.mjs` (Node.js, zero deps) or `cvis-e2e` (Go) handle all of this automatically.\n\n")

	fmt.Fprintf(&b, "## 5. Read the full skill documentation\n\n")
	fmt.Fprintf(&b, "Fetch the complete capability reference:\n\n")
	fmt.Fprintf(&b, "```\n")
	fmt.Fprintf(&b, "GET $CLAWVISOR_URL/skill/SKILL.md\n")
	fmt.Fprintf(&b, "```\n\n")
	fmt.Fprintf(&b, "This documents the task-scoped authorization model, all gateway request fields,\n")
