...
func (h *OnboardingHandler) Setup(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Fprintf(&b, "```\n\n")
	fmt.Fprintf(&b, "If this request fails (404, timeout, or empty body), **stop and tell the user**\n")
	fmt.Fprintf(&b, "that the Clawvisor daemon could not serve the skill file — they may need to\n")
	fmt.Fprintf(&b, "restart the daemon or check the relay connection. Do not continue setup without it.\n\n")
