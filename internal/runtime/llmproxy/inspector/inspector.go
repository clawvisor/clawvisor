...
// minRealisticPlaceholderLen is the shortest credential-bearing
// placeholder we treat as plausibly real. Real placeholders generated
// by autovault.GeneratePlaceholder include a 16-char base64 random
// suffix on top of an `autovault_<service>_` prefix, so genuine
// tokens are always ≥28 chars. Anything shorter is a test stub, prose
// example, or documentation fragment — present in source files but
// not actually a vault reference. Used by AllPlaceholdersAreStubs to
// suppress "ambiguous credentialed call refused" verdicts on tool_uses
// (notably Edit/Write of test or doc files) whose only `autovault_…`
// substrings are too short to resolve in the vault anyway.
//
// The suffix length is intentionally a soft check: tightening it
// would make the inspector reject placeholders the agent legitimately
// pasted in but that the LLM tokenizer truncated by a char or two
// (production has seen this happen). Threshold sits a bit below the
// nominal length so a one-character truncation still classifies the
// token as "looks real," letting the resolver be the authority on
// whether it's actually registered.
const minRealisticPlaceholderLen = len("autovault_x_") + 12
...
// should treat the tool_use as if the trigger had missed — refusing
// the surrounding tool call would just be punishing the agent for
// editing a file that happens to mention a stub.
func AllPlaceholdersAreStubs(placeholders []string) bool {
	if len(placeholders) == 0 {
		return false
	}
	for _, p := range placeholders {
		if len(p) >= minRealisticPlaceholderLen {
			return false
		}
	}
	return true
