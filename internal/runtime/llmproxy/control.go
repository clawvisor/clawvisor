...
func ControlNoticeWithCredentialHints(controlBaseURL string, availableTools []string, credentialHints []CredentialHint) string {
	...
	return strings.Join([]string{
		...
		"  - `required_credentials_json`: include only when credentials are needed. Every entry MUST include `vault_item_id` or `vault_item_handle` AND `why`.",
		"  - Invalid credential request: `\"required_credentials_json\":[{\"vault_item_id\":\"agentphone\"}]`",
		"  - Valid credential request: `\"required_credentials_json\":[{\"vault_item_id\":\"agentphone\",\"why\":\"Authenticate to agentphone to perform the approved task.\"}]`",
		"",
