...
func controlNotice(controlBaseURL string, availableTools []string, toolRules []*store.RuntimePolicyRule) string {
	...
	return strings.Join([]string{
		...
		"  - `lifetime`: omit or set `\"session\"` for a one-shot task with a fixed `expires_in_seconds`; set `\"sliding\"` for a long-running workflow whose end time you can't predict (multi-step pipelines, polling loops, anything where you want the deadline to auto-extend by 10 min on every authorized tool_use as long as you're still active); set `\"standing\"` when the user asks for persistent permission OR when they describe a RECURRING activity (\"each morning I…\", \"every PR I…\", \"as part of my daily/weekly…\") so follow-up requests reuse the same scope without re-approving. Standing tasks do not expire, so NEVER include `expires_in_seconds` with `\"lifetime\":\"standing\"`.",
		"",
