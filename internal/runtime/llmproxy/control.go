...
func controlNotice(controlBaseURL string, availableTools []string, toolRules []*store.RuntimePolicyRule) string {
	...
	return strings.Join([]string{
		...
		"",
		"Create a task before multi-step work, writing files, making network calls, changing state, or using credentials, unless every required tool call is allowed without a task. Create the task BEFORE running any permitted setup calls that are part of executing the request, so the user approves once up front rather than after substantial work; permitted read-only calls used solely to scope an accurate task spec may run beforehand. Don't wait for a tool call to be refused before creating a task.",
		"",
		"Examples:",
