...
func controlNotice(controlBaseURL string, availableTools []string, toolRules []*store.RuntimePolicyRule) string {
	...
	return strings.Join([]string{
		...
		"",
		"Field rules:",
		"  - `expected_tools`: use actual available tools (" + toolExamples + "). List plausible tools up front; include verify/read commands in the same tool `why`. If you'll likely stage intermediate data (ID lists, message dumps, working JSON) in scratch files under /tmp or the workspace, declare the file-writing tool (Write/Edit/Bash with redirects) up front rather than discovering the need mid-task — adding a tool surface after approval is scope drift and forces a re-approval.",
