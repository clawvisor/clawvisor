...
func controlNotice(controlBaseURL string, availableTools []string, toolRules []*store.RuntimePolicyRule) string {
	...
	return strings.Join([]string{
		...
		"  - If you do NOT have a placeholder for the needed service in this conversation (including: the user mentions one from a prior session, you remember one from training, the value is in the user's claimed `expected_tools`, or you're simply unsure), treat it as not present. Do NOT refuse the user's request and do NOT pass the suspect string anywhere. Instead, recover: GET " + vaultItemsURL + " to discover the real vault item, declare its account-scoped id (e.g. `github:ericlevine`, not bare `github`) in `required_credentials` of a fresh task, and use the placeholder Clawvisor mints back. The recovery is the correct response — not a refusal.",
		"  - GET " + vaultItemsURL + " is the FIRST step whenever the user names a service without naming a specific handle (\"my GitHub\", \"my work Gmail\", etc.). Don't guess handle shapes — list and pick.",
		...
		"",
		"VAULT PLACEHOLDERS — values matching `autovault_*` only exist when Clawvisor mints them in response to a `required_credentials` entry on an approved task this conversation. Use those minted values directly in Authorization headers or curl arguments — Clawvisor substitutes the real secret at proxy time. NEVER write your own `autovault_<service>` string into `required_credentials` (use the account-scoped vault item id there) or into a downstream call (use the minted placeholder). Raw tokens such as `ghp_...` or `sk-...` are sensitive; ask the user to vault them first.",
		"",
