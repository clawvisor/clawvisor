...
func controlNotice(controlBaseURL string, availableTools []string, toolRules []*store.RuntimePolicyRule) string {
	...
	scriptSessionURL := "https://" + ControlSyntheticHost + ControlSyntheticPath + "/autovault/script-session"
	...
	return strings.Join([]string{
		...
		"",
		"CREDENTIALED FAN-OUT — the trigger isn't \"am I writing a shell script\"; it's \"am I about to make ≥ 3 credentialed requests to the SAME host using the SAME placeholder under the SAME task\" (e.g. listing then fetching N Gmail message metadatas, paging through GitHub issues, etc.). When that's true, do NOT do it as N separate `curl` tool calls — each one costs a fresh inspector/rewriter round-trip and wastes ~1s/call. The flow is: (a) make the discovery call FIRST as a normal one-shot credentialed curl (the list/search call), so you know the exact fan-out size N, then (b) mint a script session sized for that N + a small buffer for retries, then (c) make the N follow-up proxy calls under the session. DO NOT mint a script session before you know how many calls you'll actually need — minting too small means hitting `SCRIPT_SESSION_EXHAUSTED` mid-flight and having to mint again (wasted approvals); minting too large makes the verifier scrutinize the request as over-broad. Steps:\n  1. As a normal one-shot curl, hit the list/search endpoint that returns the IDs / page count / batch size you're about to fan out over. Record N.\n  2. POST " + scriptSessionURL + " with `{placeholder, target_host, methods, path_prefixes, max_uses: N + small_buffer, ttl_seconds, why}`. Pick `max_uses` to match the actual N you discovered (plus ~2-3 for retries), not the hard cap. The response gives `caller_token` + `base_url`.\n  3. From your script (or N tool calls if you prefer), call `base_url + <upstream-path>` with `Authorization: Bearer <placeholder>`, `X-Clawvisor-Target-Host: <target_host>`, `X-Clawvisor-Caller: Bearer <caller_token>` on EACH request.\nHard limits: TTL ≤ 120s, max_uses ≤ 50, GET-only initially, one target_host + one placeholder per session, response bytes capped per-request (1 MiB) and total (10 MiB). When a session is exhausted/expired/out-of-scope the proxy returns a structured 403/401 with codes like `SCRIPT_SESSION_EXHAUSTED`; mint a fresh session rather than retrying. See GET " + scriptDocsURL + " for the full request shape and error recovery. One-shot credentialed calls (single message fetch, one-off API hit) DO NOT need a script session — keep using direct `curl` for those.",
		"",
