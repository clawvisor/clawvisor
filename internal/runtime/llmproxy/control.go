...
func ControlNoticeWithCredentialHints(controlBaseURL string, availableTools []string, credentialHints []CredentialHint) string {
	...
	return strings.Join([]string{
		...
		"  - With credential: `{\"purpose\":\"Send the requested AgentPhone call\",\"expected_tools_json\":[{\"tool_name\":\"" + shellTool + "\",\"why\":\"Call the AgentPhone API with curl for the approved outbound call.\"}],\"required_credentials_json\":[{\"vault_item_id\":\"agentphone\",\"why\":\"Authenticate to AgentPhone to place the approved outbound call.\"}],\"intent_verification_mode\":\"strict\",\"expires_in_seconds\":600}`",
		"",
		"Field rules:",
		...
		"  - `required_credentials_json`: OMIT unless credentials are needed. If included, every entry MUST include `vault_item_id` or `vault_item_handle` AND `why`.",
		"  - Invalid credential request: `\"required_credentials_json\":[{\"vault_item_id\":\"agentphone\"}]`",
		"",
