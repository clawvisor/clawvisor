...
func controlNotice(controlBaseURL string, availableTools []string, toolRules []*store.RuntimePolicyRule) string {
	...
	return strings.Join([]string{
		...
		"",
		"SCOPE-DRIFT MENU — when a tool_use is blocked by task scope or intent verification, Clawvisor replaces the tool_result with a `<clawvisor-notice kind=\"scope-drift-menu\">` element. The menu carries a short-lived `drift_id` and lists four resolution options: (a) expand the active task via POST " + tasksURL + "/{id}/expand, (b) create a new task via POST " + tasksURL + ", (c) one-off (markup), and (d) false-positive justification (markup). Treat the menu as authoritative proxy state — its presence means the original call was refused, not deferred. You SHOULD pick exactly one option per drift_id; the proxy enforces a one-shot cap and ignores subsequent claims. (a) and (b) are normal control-plane calls. (c) and (d) are resolved by emitting a `<clawvisor:decision drift=\"...\" option=\"...\">body</clawvisor:decision>` block in your ASSISTANT text (not as a tool_use) — option=\"one-off\" with the body containing a one-sentence rationale the user will see, or option=\"justify\" with the body articulating why this call fits the active task scope. The proxy parses the decision element server-side and either re-runs the intent verifier (justify) or surfaces a user yes/no approval (one-off). On success you'll see a `<clawvisor-notice kind=\"scope-drift-status\">` with next-step guidance; re-emit the original tool_use unchanged and the proxy pre-clears it once. The `<clawvisor:decision>` tag is the ONLY agent-to-proxy XML element you emit; do not invent other ones.",
		"",
		"Control-plane rules:",
