...
// Org returns an org-scoped (non-governance) route for sub (e.g. "sso" or
// "settings"). Only meaningful when OrgID is set — these are Clawvisor Cloud /
// enterprise routes with no instance-scoped equivalent, so callers must verify
// OrgID first.
